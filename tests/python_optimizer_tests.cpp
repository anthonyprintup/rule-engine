...
namespace {
    ...
    TEST_CASE("provider scan adapters preserve supported spaces through protocol v2") {
        ...
        for (const auto kind : kinds) {
            ...
            plan.context_bytes_before = 0;
            plan.context_bytes_after = 0;
    ...
    TEST_CASE("provider response adapters preserve exact matches and reject lossy shapes") {
        const auto literal = make_byte_pattern("mz", bytes("MZ"));
        REQUIRE(literal.has_value());
        auto plan = plan_with(*literal, 4, 8);
        plan.context_bytes_before = 0;
        plan.context_bytes_after = 0;
        ProviderScanRequest request {
            ...
            .subject = SubjectKey {.peer = PeerId {"peer-1"},
                                   ...
                                   .identity = {{.field_id = 1, .value = std::uint64_t {42}}},
                                   .parent = {}},
            .space = file_space(0x1000, 4),
            .plan = plan,
            .deadline_unix_ms = 5'000,
        };
        const auto source = bytes("MZMZ");
        ...
        const auto restored = from_contract_scan_response(request, *response);
        REQUIRE(restored.has_value());
        REQUIRE(restored->matches.size() == matches->matches.size());
        CHECK(restored->matches[0].pattern_id == "mz");
        CHECK(restored->matches[0].absolute_address == 0x1000);
        CHECK(restored->matches[1].absolute_address == 0x1002);
        ...
        auto context_request = request;
        context_request.plan.context_bytes_before = 1;
        const auto context_result = to_contract_scan_request(context_request);
        REQUIRE_FALSE(context_result.has_value());
        CHECK(context_result.error().code == ScanWireErrorCode::unsupported_shape);

        auto multi_pattern_request = request;
        multi_pattern_request.plan.patterns.push_back(*literal);
        multi_pattern_request.plan.patterns.back().pattern_id = "mz-second";
        const auto multi_pattern_result = to_contract_scan_request(multi_pattern_request);
        REQUIRE_FALSE(multi_pattern_result.has_value());
        CHECK(multi_pattern_result.error().code == ScanWireErrorCode::unsupported_shape);
    }
