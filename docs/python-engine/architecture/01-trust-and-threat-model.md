...
- Agent fact/scan replies are untrusted authenticated inputs until session, fence, request, subject, route, schema, label, and size validation completes.
...
| Provider response | Predicate injection, wrong route/schema/subject, duplicate response, invalid status/value, oversized scan context | Provider API contains no predicate/verdict type; correlate request/fence/subject/route; validate schema/status/size; reject protocol violations atomically. |
