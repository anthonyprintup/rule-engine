...
- Runtime and activation state have durable SQLite/PostgreSQL adapters, and the production session backend now grants bounded durable-message credit backed by the atomic agent-ingress transaction. Startup still has no activated event-to-compiled-pack scheduler, snapshot projector, or work-result-to-VM continuation. Sessions therefore accept and durably acknowledge authoritative snapshot records but advertise no detection work. The remaining composition gap is tracked in [L-031](LIMITATIONS.md#l-031--resident-agent-ingress-is-durable-but-not-yet-scheduled).
...
The missing activated snapshot/work scheduler, remaining admin
