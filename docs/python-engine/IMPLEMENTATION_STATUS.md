...
- The generic VM contract still lacks typed emitted-event payloads. `balanced.v1` has no separate cumulative logical-allocation ceiling; cross-retry allocation usage is reported and overflow-safely aggregated under [L-029](LIMITATIONS.md#l-029--logical-allocation-has-no-distinct-balancedv1-ceiling).
