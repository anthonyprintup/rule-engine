...
- **Observability:** Stable startup errors identify unavailable plaintext/TLS paths; scheduler counters expose queued/active/rejected sessions and listener failures; backend/store metrics should measure empty work polls and delivery latency, but dedicated exported poll metrics are not implemented; process supervision records termination and RSS.
- **Revisit:** Add an explicitly bounded plaintext development listener if still required, independent accept/handshake workers, database-backed work notification with bounded polling fallback, dedicated poll/delivery metrics, and measured allocator/process limits without weakening lease or shutdown guarantees.
