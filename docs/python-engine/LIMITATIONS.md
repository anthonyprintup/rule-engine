...
- **Impact:** The agent publishes an immediate authoritative process snapshot and refreshes it every configured interval, but agents with identical start times and settings are not fleet-jittered. A very short fleet-wide cadence can therefore create synchronized enumeration and ingress load. On Windows, `NtQuerySystemInformation` is synchronous: once a native enumeration call has entered the kernel, a stop request or deadline cannot interrupt that individual call, so shutdown latency can exceed the configured application deadline if Windows does not return promptly.
- **Rationale:** One synchronous deadline-driven session owner preserves socket ordering, cancellation, and the single-writer spool boundary. The scheduler waits for either readable input or the next inventory deadline and never starts overlapping enumerations. The implementation does not claim that a stop token can make the native Windows API killable.
- **Mitigation:** `inventory_refresh_interval_ms` is mandatory and accepts only 1,000 through 86,400,000 milliseconds. Each successful enumeration becomes a complete begin/chunk/commit batch, is validated, and is inserted into SQLite atomically before first transmission. Its checked monotonic inventory generation combines the active runtime generation with the durable spool sequence, starting above the legacy generation-only scheme so reconnect, upgrade, and process restart cannot reuse an acknowledged generation. Failed or partial enumeration publishes no batch, preserving the coordinator's last-good authority; pending rows replay normally after reconnect. Enumeration checks stop/deadline immediately before and after every native query and retry and throughout bounded parsing, so a late or canceled result is discarded. Run the agent under a service supervisor with a finite shutdown grace and force-terminate the agent process after that grace if the native query remains blocked; SQLite WAL recovery preserves already committed spool records.
- **Observability:** An internal/test seam records refresh attempts, authoritative generations spooled, refreshes without authority, snapshot records, connections, and replay. The production executable does not yet export these counters; operators currently observe coordinator snapshot ingress, the durable spool, and process supervision. The absence of a new authoritative commit is never interpreted as an empty inventory.
- **Revisit:** Add bounded deterministic fleet jitter and exported cadence/lag metrics after production fleet-load measurements define an acceptable distribution policy. A genuinely hard in-process shutdown bound requires moving enumeration behind a separately supervised killable process boundary; a thread or stop token alone is insufficient.

## L-025 — The Windows agent does not check server-certificate revocation online
...
- **Mitigation:** Every production configuration must contain `require_crl = true` and a normalized fixed-local-drive `crl_path`. Relative, UNC, mapped-drive, alternate-stream, and reparse-point paths fail closed. The bounded file must contain only PEM CRLs, so it cannot inject certificates into the configured trust store. Configuration validation rejects missing, malformed, not-yet-valid, or expired CRLs before dialing; full-chain verification rejects missing issuer coverage, a wrong issuer, or a revoked server certificate during the bounded TLS handshake. Publish refreshed CRLs atomically and restart agents before `nextUpdate`; continue rotating the exact fingerprint and trust bundle through controlled deployment.
...
- **Impact:** Authors can declare `@schema`-pinned `EventRecord` classes, assign explicit positive `wire_field` IDs, omit fields with type-correct canonical `bool`, `int`, `float`, `str`, or `bytes` constructor defaults, and call `telemetry.emit(record)` as a standalone statement. Defaulted fields remain present and wire-required; defaults do not provide optional-field schema evolution. Dynamic and aggregate defaults, positional construction, and use of the returned `EventReceipt` inside rule bytecode are rejected. Projected envelopes retain root causation only and reuse the root ingest timestamp because the durable contract has no immediate-parent/depth or commit-clock fields. Evaluation records written before the event-journal codec marker require an explicit migration or development-store rebuild.
...
- **Mitigation:** The compiler derives `SchemaKind::event` only from the recognized `EventRecord` base plus one literal stable `@schema` decorator, validates unique field IDs, field types, and scalar defaults, materializes every omitted default before a verifier-pinned `build_record`, then emits `emit_event`. These semantics select `python-3.14.6/static-compiler-v2`; legacy v1 manifests and resident compilation identities are rejected before compilation or activation. The VM reconstructs fields from the active descriptor, validates and freezes the record, and every durable store cross-checks deterministic intent identity, order, inherited context, causation, timestamps, label, and exact payload before commit. Missing required fields and unsupported constructor or receipt usage fail compilation. Operators must migrate or recreate pre-marker development result rows before upgrading.
