...
- **Impact:** Development configuration can request loopback plaintext, but the resident service currently refuses startup unless both application listeners have authenticated TLS contexts. TCP accept plus TLS handshake is serial in the node loop before a session enters the bounded worker queue. The agent backend is polled initially and after each durable inbound message, but there is no independent idle-session work notification/poll; work created without an inbound message waits for reconnect or later traffic. The configured memory reservation covers application-owned frame/queue estimates, not total process RSS; OS/OpenSSL/allocator/backend allocations remain outside it. With project exceptions disabled, an unrecoverable thread/allocation failure terminates the process.
...
- **Mitigation:** Use mTLS even for development, keep accept/handshake deadlines below the validated lease window, bound the queue/workers/session lifetime, apply an external process/container memory limit, and reconnect or send bounded session traffic to obtain work created while idle.
