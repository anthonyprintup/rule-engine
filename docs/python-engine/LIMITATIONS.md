...
- **Revisit:** Add authenticated migration/reset/retained-gap state strategies only after their runtime state transitions are composed; add versioned resident capability inventory, registry maintenance metrics/audit and an object-store adapter, platform event adapters, phase-aware worker cancellation, an alternative atomic publication protocol, and authorized optimistic policy-record mutation with semantic canonicalization and migration tooling.
