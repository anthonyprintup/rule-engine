...
## L-031 — Resident agent ingress is durable but not yet scheduled

- **Impact:** The production listener can now durably accept and cumulatively acknowledge authoritative snapshot records under bounded credit. It still advertises no detection work because startup does not load the activated source pack into a resident VM scheduler, project committed snapshots into evaluation events, or resume an evaluation from a returned work result. The bounded admin wire supports pack/operation reads and final activation flip only; the standalone admin CLI has no network adapter.
- **Rationale:** A cumulative ACK permits deletion from the agent's durable spool, so the body and receipt now commit atomically under the current session lease. Dispatch remains separate: issuing synthetic work without the exact activated compiled pack, binding, subject event, and generation fence would move semantic ownership outside C++ and could run stale rules.
- **Mitigation:** `IResidentAgentBackend::persist` encodes the validated durable body and calls one store transaction that checks the `(tenant, peer, epoch)` sequence and active session lease, writes the body, advances the receipt, and appends durable audit before commit. Exact retries are idempotent; changed replays, gaps, expired sessions, and injected pre-commit failures do not advance ACK. Until the activated scheduler exists, `take_work` returns no leases.
- **Observability:** Runtime-store inspection exposes committed agent messages; the cumulative receipt is independently queryable; durable audit records `agent.message.commit`. Stable store/protocol errors distinguish stale sessions, gaps, malformed replay, and persistence failure. Scheduler snapshots still expose accepted, queued, active, rejected, and stopping session counts.
- **Revisit:** Load and verify active source packs at startup, compare their exact compiler/binding/executable identities with durable activation reports, rebuild snapshot projection from durable ingress, and persist/lease evaluation work through completion. Then wire the remaining typed admin operations and CLI transport. The configured memory bound covers owned application reservations; OS socket buffers, TLS-library allocations, allocator overhead, and backend-internal memory require separate process/container limits and measurement.
